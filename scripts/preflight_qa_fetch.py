...
class Preflight:
    ...
    def build_packet_for_goal(self, goal: GoalInfo) -> bool:
        ...
        task = {
            ...
            "subagent": {
                "execution_mode": "acceptance_only",
                "prompt_file": f"{self.packet_root.relative_to(self.repo_root).as_posix()}/{goal.goal_id}/task_prompt.md",
            },
            "allow_noop": True,
        ...
        for name in REQUIRED_VALIDATOR_CHECKS:
            check_pass = True if acceptance_ok else (name != "acceptance_commands_executed")
            if name == "codex_invoked":
                check_pass = True
            if name == "changed_files_non_empty":
                check_pass = True
            if name == "changed_files_within_allowed_paths":
                check_pass = True
            checks.append(
                {
                    "name": name,
