...
def fetch_stock_day(
    ...
    *,
    ...
    """Fetch stock daily bars from MongoDB (QUANTAXIS-compatible)."""
    db = get_db()
    ...
    proj = None
    if fields:
        base_fields = set(fields) | {"code", "date", "vol", "volume", "amount", "open", "high", "low", "close"}
    ...
    if "vol" in res.columns and "volume" not in res.columns:
        res = res.assign(volume=res.vol)
    elif "volume" in res.columns and "vol" not in res.columns:
        res = res.assign(vol=res.volume)
    ...
    try:
        ...
        if "volume" in res.columns:
            res = res.query("volume>1")
        res = res.set_index("date", drop=False)
        ordered = ["code", "open", "high", "low", "close", "volume", "amount", "date"]
