...
def _ensure_wequant_env_defaults() -> None:
    defaults = {
...
@lru_cache(maxsize=1)
def _load_wequant_module() -> ModuleType:
    _ensure_wequant_env_defaults()
...
def resolve_wequant_callable(func_name: str) -> Callable:
    _ensure_wequant_env_defaults()
    try:
        from .providers.function_impl import resolve_mongo_fetch_callable

        return resolve_mongo_fetch_callable(func_name)
    except Exception:
        pass

    mod = _load_wequant_module()
    fn = getattr(mod, func_name, None)
    if callable(fn):
        return fn

    # Fallback to submodules for partial exports.
    for mod_name in (
