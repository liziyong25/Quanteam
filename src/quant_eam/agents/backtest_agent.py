...
def _run_fetch_request(*, payload: dict[str, Any], out_dir: Path) -> tuple[dict[str, Any], list[Path]]:
    ...
    mode = str(fetch_request.get("mode", "backtest") or "backtest")
...
def run_backtest_agent(*, input_path: Path, out_dir: Path, provider: str = "mock") -> list[Path]:
    if str(provider).strip() != "mock":
        raise ValueError("provider 'external' is not supported in backtest_agent MVP")
    payload = _load_json(Path(input_path))
    if not isinstance(payload, dict):
        raise ValueError("backtest_agent input must be a JSON object")
    fetch_meta, fetch_files = _run_fetch_request(payload=payload, out_dir=Path(out_dir))

    run_plan = {
        "schema_version": "backtest_agent_plan_v1",
        "agent_id": "backtest_agent_v1",
        "job_id": payload.get("job_id"),
        "runspec_path": payload.get("runspec_path"),
        "policy_bundle_path": payload.get("policy_bundle_path"),
        "trace_preview_path": payload.get("trace_preview_path"),
        "fetch": fetch_meta,
        "notes": [
            "Backtest execution is deterministic and delegated to orchestrator runner/gaterunner.",
            "This plan is evidence only; it does not execute trading logic directly.",
        ],
    }
    out_path = Path(out_dir) / "backtest_plan.json"
