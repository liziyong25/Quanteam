...
def _sync_fetch_evidence_into_dossier(*, outputs_dir: Path, dossier_dir: Path) -> dict[str, Any]:
    source_dir = outputs_dir / "fetch"
    if not source_dir.is_dir():
        return {"synced": False, "reason": "source_fetch_dir_missing"}

    dossier_dir.mkdir(parents=True, exist_ok=True)
    target_dir = dossier_dir / "fetch"
    ...
    for src in sorted(source_dir.iterdir(), key=lambda p: p.name):
        if not src.is_file():
            continue
        name = src.name
        if name in canonical_names or name.startswith("step_"):
            dst = target_dir / name
            shutil.copy2(src, dst)
            copied.append(name)

    if not copied:
        return {"synced": False, "reason": "no_fetch_files_to_copy", "source_dir": source_dir.as_posix()}

    steps_index_path = target_dir / "fetch_steps_index.json"
    rewritten_paths = 0
    if steps_index_path.is_file():
        try:
            idx_doc = json.loads(steps_index_path.read_text(encoding="utf-8"))
        except Exception:
            idx_doc = {}
            ...
            if isinstance(rows, list):
                for row in rows:
                    ...
                    for key in ("request_path", "result_meta_path", "preview_path", "error_path"):
                        ...
                        if cand.is_file():
                            row[key] = cand.as_posix()
                            rewritten_paths += 1
                steps_index_path.write_text(json.dumps(idx_doc, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    if not isinstance(artifacts, dict):
        artifacts = {}
    key_map = {
        "fetch_request.json": "fetch_request",
        "fetch_result_meta.json": "fetch_result_meta",
        "fetch_preview.csv": "fetch_preview",
        "fetch_error.json": "fetch_error",
        "fetch_steps_index.json": "fetch_steps_index",
    }
    for name in copied:
        mapped_key = key_map.get(name)
        if mapped_key:
            artifacts[mapped_key] = f"fetch/{name}"
    manifest_doc["artifacts"] = artifacts

    hashes = manifest_doc.get("hashes")
    ...
    out: dict[str, Any] = {
        ...
        "dossier_fetch_dir": target_dir.as_posix(),
        "copied_count": len(copied),
        "copied_files": copied,
        "rewritten_step_paths": rewritten_paths,
    }
    if steps_index_path.is_file():
        out["dossier_fetch_steps_index_path"] = steps_index_path.as_posix()
    return out
    ...
    while True:
        ...
        if schema_version == "idea_spec_v1":
            ...
            if not _has_event(events, "RUN_COMPLETED"):
                ...
                dossier_fetch_sync: dict[str, Any] = {}
                if dossier_path:
                    dossier_fetch_sync = _sync_fetch_evidence_into_dossier(
                ...
                run_link_doc = {
                    ...
                    "dossier_fetch_sync": dossier_fetch_sync,
                ...
                write_outputs_index(
                    ...
                    updates={
                        ...
                        "dossier_fetch_sync": dossier_fetch_sync,
                        "dossier_fetch_dir": dossier_fetch_sync.get("dossier_fetch_dir"),
                        "dossier_fetch_steps_index_path": dossier_fetch_sync.get("dossier_fetch_steps_index_path"),
                    },
                )
                append_event(
                    job_id=job_id,
