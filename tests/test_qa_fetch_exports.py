...
def test_qa_fetch_exports_have_priority_and_wb_fallback_alias() -> None:
    ...
    assert export_map["fetch_future_day"]["source"] == "mongo_fetch"

    assert "wb_fetch_bond_day" in export_map
    assert export_map["wb_fetch_bond_day"]["source"] == "mysql_fetch"
...
def test_qa_fetch_exports_include_resolver_runtime_apis() -> None:
    ...
    assert callable(qa_fetch.execute_ui_llm_query)
    assert callable(qa_fetch.execute_ui_llm_query_path)
