...
3. Fetch-contract schema exception window is pre-authorized in unattended mode.

- Source of truth: `autopilot_stop_condition_exceptions_v1` in `docs/12_workflows/skeleton_ssot_v1.yaml`
- Meaning: `Any change touching contracts/**` may be waived only for fetch-contract goals with explicit exception scope.
- Allowed contract files are limited to:
  - `contracts/fetch_request_schema_v1.json`
  - `contracts/fetch_result_meta_schema_v1.json`
- Redlines still apply: no edits under `policies/**`, no holdout visibility expansion, and no non-fetch contract expansion.
...
- Acceptance commands
- Required skills and enforcement mode
- Reasoning tier/runtime profile
- UI black-box checks
...
### 6) Capability-Cluster Milestone (Autopilot v2)

When a cross-document capability cluster reaches done state, Orchestrator performs milestone flow:

1. Re-evaluate cluster completion:
- all cluster goals are `implemented`
- all `requirements_trace_v1` rows in the cluster are `implemented`
- latest cluster phase packet passes `check_subagent_packet.py`

2. Run strict gate before commit/push:
- `python3 scripts/check_docs_tree.py`
- `python3 scripts/check_subagent_packet.py --phase-id <latest_phase_id>`
- cluster acceptance commands and target tests must pass
- whitelist-only commit scope in dirty workspace
- redline deny by default: `contracts/**`, `policies/**`, holdout visibility expansion

3. Commit/push policy:
- stage only explicit whitelist paths (never `git add .`)
- try `git push origin master`
- if rejected, fallback push `origin/autopilot/milestone-<milestone_id>`

4. Audit writeback:
- append `milestone_history_v1` in SSOT
- write `artifacts/autopilot/milestones/<milestone_id>/milestone_summary.md`
- write `artifacts/autopilot/milestones/<milestone_id>/commit_manifest.json`

5. Queue-empty continuation protocol:
- when no `planned/partial/in_progress` goals remain, execute
  `docs/12_workflows/whole_view_autopilot_queue_empty_protocol_v1.md`
- auto-generate one new `skeleton` goal + one new `impl_fetchdata` goal
- write back SSOT before next dispatch cycle
- if `queue_empty_generation_protocol_v1.allow_generate_after_done=true`, generation may continue
  after done criteria, otherwise done-guard stops generation
- in controller loop mode, keep cycling until:
  - done criteria satisfied, or
  - a goal becomes blocked after retry exhaustion
