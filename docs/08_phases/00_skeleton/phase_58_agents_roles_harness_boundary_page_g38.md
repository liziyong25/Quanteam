...
- `docs/08_phases/00_skeleton/phase_58_*.md`
...
- Subagent 禁止修改 `docs/12_workflows/skeleton_ssot_v1.yaml`；仅主控在全部验收通过后回写 `G38.status_now=implemented`。
...
- Notes:
  ...
  - Implemented `GET/HEAD`-only route `/ui/agent-roles` in `src/quant_eam/api/ui_routes.py`, sourcing evidence from:
    ...
    - `docs/12_workflows/skeleton_ssot_v1.yaml` section `agents_pipeline_v1`
