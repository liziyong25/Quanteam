...
- Subagent 禁止修改 `docs/12_workflows/agents_ui_ssot_v1.yaml`；仅主控在全部验收通过后回写 `G42.status_now=implemented`。
...
- Notes:
  ...
  - Evidence rendering is sourced from:
    ...
    - `docs/12_workflows/agents_ui_ssot_v1.yaml` entries `goal_checklist(G42)` / `phase_dispatch_plan_v2` / `g42_diagnostics_promotion_ui_scope` / `agents_pipeline_v1`
